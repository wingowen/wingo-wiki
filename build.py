...
# Only run build steps when executed as a script (not imported as a module).
# This allows generate_graph.py to `from build import CATEGORY_MAP, WIKILINK_MAP`
# without triggering the full build pipeline.
if __name__ == "__main__":
    ...
    for short_name, (cat, subcat) in CATEGORY_MAP.items():
        ...
        if not src_file.exists():
            src_file = WIKI / "entities" / short_name
        if not src_file.exists():
            src_file = WIKI / "queries" / short_name
        if not src_file.exists():
            src_file = WIKI / "comparisons" / short_name
        if not src_file.exists():
            src_file = WIKI / "concepts" / short_name  # root-level concepts dir
    ...
    for dir_name in DOCS_DIRS:
        ...
        for doc_file in scan_dir.rglob("*.md"):
            ...
            for match in link_pattern.finditer(content):
                ...
                link_url = path_to_url(os.path.relpath(os.path.join(os.path.dirname(doc_file), link_path), DOCS))
    ...
    print(f"  Generated {len(nodes_list)} nodes and {len(links_list)} links")
    ...
    Graph:       docs/assets/graph-data.json ({len(nodes_list)} nodes, {len(links_list)} links)
